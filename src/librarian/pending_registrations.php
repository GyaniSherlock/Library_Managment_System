			...
			if(isset($_POST['l_confirm']))
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					if(isset($_POST['cb_'.$i]))
					{
						...
						$from='noreply2libmangement@gmail.com';
						$password='lib123manage';
						$sub='Registration Request Accepted';$content=$message;
			...
			if(isset($_POST['l_delete']))
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					if(isset($_POST['cb_'.$i]))
					{
						...
						$from='noreply2libmangement@gmail.com';
						$password='lib123manage';
						$sub='Regisitration Request Rejected';$content=$message;
						$mail = new PHPMailer\PHPMailer\PHPMailer();
						$mail->isSMTP();
						$mail->SMTPAuth = true; // authentication enabled
						$mail->SMTPSecure = 'ssl'; // secure transfer enabled REQUIRED for Gmail
